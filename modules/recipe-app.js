var PUBLISHER_CONFIG_PATH = '/_system/config/recipe-app/configs/recipe-app.json';

var ASSETS_EXT_PATH = '/extensions/assets/';

var TENANT_PUBLISHER = 'tenant.recipe-app';
var log = new Log('modules.recipe-app');
...
var init = function (options) {
    ...
    event.on('tenantCreate', function (tenantId) {
        var role, roles,
            ...
            mod = require('store'),
            server = mod.server,
            config = require('/config/recipe-app-tenant.json'),
    ...
    event.on('tenantLoad', function (tenantId) {
        ...
        var recipe_appConfig = require('/config/recipe-app-tenant.json');
        ...
        if (tenantId == SUPER_TENANT) {
            ...
            var deploymentManager = new deployer.Deployer({
                config: recipe_appConfig.defaultAssets
            });
...
var recipe_app = function (o, session) {
    var recipe_app, tenantId, store,
        server = require('store').server;
    tenantId = (o instanceof Request) ? server.tenant(o, session).tenantId : o;
    recipe_app = session.get(TENANT_PUBLISHER);
    if (recipe_app) {
        return recipe_app;
    }
    recipe_app = new Publisher(tenantId, session);
    session.put(TENANT_PUBLISHER, recipe_app);
    return recipe_app;
};
...
var currentAsset = function () {
    var prefix = require('/config/recipe-app.js').config().urls.ASSETS,
        matcher = new URIMatcher(request.getRequestURI());
    if (matcher.match('/{context}' + prefix + '/{type}/{+any}') || matcher.match('/{context}' + prefix + '/{type}')) {
        return matcher.elements().type;
    }
    prefix = require('/config/recipe-app.js').config().urls.EXTENSIONS;
    if (matcher.match('/{context}' + prefix + '/{type}/{+any}') || matcher.match('/{context}' + prefix + '/{type}')) {
        return matcher.elements().type;
    }
    return null;
...
/*
 ...
 2. Assign a set of default roles (private_username and recipe-app)
...
var exec = function (fn, request, response, session) {
    ...
    es.server.sandbox({
        ...
        //var configs = require('/config/recipe-app.js').config();
        return fn.call(null, {
            //tenant: tenant,
            //server: es.server,
            //usr: es.user,
            //user: user,
            //recipe-app: require('/modules/recipe-app.js').recipe-app(tenant.tenantId, session),
            //configs: configs,
            request: request,
            response: response,
            session: session,
